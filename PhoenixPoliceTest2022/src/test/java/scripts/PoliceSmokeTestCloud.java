...
public class PoliceSmokeTestCloud {
...
{
	testContext.setAttribute("moduleName", "CloudSmokePolice");
...
//Mahesha
    ...
    public void AlarmModuleSmokeTestScriptAlarm(ITestContext testContext) throws Exception
    {
      ...
      for(int i=1;i<=cou;i++)
      {
    	  objectIdentification.smokeTest.handlingAcknowledgeMessagesPopups( PoliceData.getPoliceSmokeModuleValue("Screen"+i+""));
        ...
        if(PoliceData.getPoliceSmokeModuleValue("Screen"+i+"").equalsIgnoreCase("Alarm Type"))
        	{
              ...
              objectIdentification.smokeTest.saveAndCloseTheScreenGlobal(PoliceData.getPoliceSmokeModuleValue("EntryScreen"+i+""));
    ...
    public void AnimalControlModuleSmokeTestScript(ITestContext testContext) throws Exception
    {
      ...
      for(int i=1;i<=cou;i++)
      {
       ...
       System.out.println(PoliceData.getPoliceSmokeModuleValue("Screen"+i+""));
        ...
        {
        	Playback.driver.findElement(By.id("tabNameSrch_tmpl0_UsrAnimalSrchBasic1_lupCFS_CodeValue")).sendKeys("742");
        	Playback.driver.findElement(By.id("tabNameSrch_tmpl0_UsrAnimalSrchBasic1_cddComplaintType_DropDown")).sendKeys("Wal");
        }
        objectIdentification.smokeTest.clickTheSearchButton();
        if (objectIdentification.windowHandle.windowExist("Acknowledge"))
        	try {
        ...
        for(int i=1;i<=cou;i++)
        {
         ...
         System.out.println(PoliceData.getPoliceSmokeModuleValue("Screen"+i+""));
          ...
          else if(PoliceData.getPoliceSmokeModuleValue("Screen"+i+"").equalsIgnoreCase("Report Search"))
          {
        	  ...
        	  Playback.driver.findElement(By.xpath("//select[contains(@id,'tabRpt_tmpl0_ctrlusrRptSrchBSC_cddRptType_DropDown')]"))
          ...
          else
          {
             try {
              ...
              objectIdentification.smokeTest.selectRandomGridRow();
              objectIdentification.windowHandle.switchToWindow(PoliceData.getPoliceSmokeModuleValue("EntryScreen"+i+""));   
              if(PoliceData.getPoliceSmokeModuleValue("EntryScreen"+i+"").equalsIgnoreCase("Report Status"))
              {
            	  Playback.driver.findElement(By.id("cmdRefresh")).click();
				...
				} catch(Exception e)
	        {
				...
				try {
					...
					if(PoliceData.getPoliceSmokeModuleValue("EntryScreen" + i + "").equalsIgnoreCase("UOF Entry")||
							PoliceData.getPoliceSmokeModuleValue("EntryScreen" + i + "").equalsIgnoreCase("DV Entry"))
					{
		...
		for (int i = 1; i <= cou; i++) {
			...
			while (res) {
				...
				else if (PoliceData.getPoliceSmokeModuleValue("Screen" + i + "").equalsIgnoreCase("Traffic Interface Log")) 
				{
					...
					objectIdentification.smokeTest.clickTheSearchButton();
					verify.ExpectedValueIsTrue(objectIdentification.manualIdentify.controlExist(Playback.driver.findElement(By.id("grdCourtHist"))));
					objectIdentification.windowHandle.switchToMainWindow();
					res = false;
					break;
				}
